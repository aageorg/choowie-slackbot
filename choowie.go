...
func main() {
	...
	flag.StringVar(&certFile, "tls-cert-path", "", "Full path to TLS certificate file.")
	flag.StringVar(&privkeyFile, "tls-privkey-path", "", "Full path to TLS private key file")
	...
	http.HandleFunc("/oAuth", func(res http.ResponseWriter, req *http.Request) {
		...
		for _, user := range authedUsers {
			...
			settings.User = user
			settings.User.TeamId = appuser.TeamId
			settings.User.IsOwner = appuser.IsOwner
			settings.User.IsAdmin = appuser.IsAdmin
			settings.User.IsBot = appuser.IsBot
			settings.User.UserName = appuser.UserName
			settings.User.Profile.ApiAppId = appuser.Profile.ApiAppId
			fmt.Printf("%+v", settings.User)
			settings.SaveToken()
		}
	...
	http.HandleFunc("/noautomove", func(res http.ResponseWriter, req *http.Request) {
		defer req.Body.Close()
	...
	http.HandleFunc("/automove", func(res http.ResponseWriter, req *http.Request) {
		defer req.Body.Close()
		body, err := io.ReadAll(req.Body)
		if err != nil {
			fmt.Println("error while reading body")
		...
		settings.User = User{Id: q.Get("user_id"), TeamId: q.Get("team_id")}
		setNewAutomove := func(message_id string, from string, to string) {
			...
			//delete(waitReactionTo, message_id)
			close(incoming_ch)
		...
		go setNewAutomove(message_id, from, to)

		return
	})

	http.HandleFunc("/showautomoves", func(res http.ResponseWriter, req *http.Request) {
		defer req.Body.Close()
		body, err := io.ReadAll(req.Body)
		if err != nil {
			fmt.Println("error while reading body")
	...
	log.Fatal(http.ListenAndServeTLS(":"+listenPort, certFile, privkeyFile, nil))
